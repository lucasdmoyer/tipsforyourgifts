...
Every generated build also carries `publication-manifest.json`, a deterministic public release-candidate ledger. It binds each publication-ready article to the exact article bytes, validated research run, separate QA receipt, reviewed evidence digest, optional completed research mission, and optional reviewed social launch pack. It also records the affiliate-registry posture and total live affiliate links. The manifest's `release_candidate` status means only that the prepared content set passed local gates; it is not a deployment claim.
